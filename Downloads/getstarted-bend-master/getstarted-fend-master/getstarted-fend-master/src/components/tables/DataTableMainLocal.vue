    ...
    <v-data-table
        ...
        dense>
          ...
          <template v-slot:item.Activo="{ item }">
            <v-chip class="text-uppercase" :color="(item.activo == 'si')? 'success':'error'" outlined label small>
              {{ item.activo }}
            </v-chip>
          </template>

          <template v-slot:item.FieldJson="{ item, header }">
            <spans> {{ events(header, item) }}</spans>
          </template>
...
export default {
  ...
  methods: {
    ...
    showfieldJson (val, item) {
      const json = JSON.stringify(item[val])
      return `${json}`
    }
  }
}
